    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="flex flex-col items-center gap-4" onClick={(e) => e.stopPropagation()}>
        <div
          ...
          style={{ backgroundColor: '#08090d', fontFamily: 'sans-serif' }}
        >
          <div className="relative w-full aspect-[2/3]">
            {src ? (
              <img src={src} alt={title} className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full flex items-center justify-center text-white/60" style={{ backgroundColor: '#14151c' }}>
                {title}
              </div>
            )}
            <div
              className="absolute inset-x-0 bottom-0 h-2/3"
              style={{ background: 'linear-gradient(to top, #08090d 20%, transparent)' }}
            />
          </div>
          ...
          <div className="px-5 pb-6 -mt-16 relative flex flex-col gap-2">
            ...
            {stars != null && (
              <div className="flex items-center gap-1">
                  ...
                  return (
                    <div key={i} className="relative w-5 h-5">
                      ...
                      <div className="absolute inset-0 overflow-hidden" style={{ width: `${fill * 100}%` }}>
                        <svg viewBox="0 0 24 24" className="w-5 h-5" style={{ color: '#f5c518' }} fill="currentColor">
                          <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                        </svg>
                      </div>
                    </div>
                ...
                <span className="ml-1 text-sm font-semibold" style={{ color: '#f5c518' }}>
                  {stars.toFixed(1)}
                </span>
              </div>
            )}
            ...
            <div className="flex items-center justify-between mt-3 pt-3" style={{ borderTop: '1px solid #212330' }}>
              <span className="text-xs text-white/50">{watchedDate}</span>
              <span className="text-sm font-bold" style={{ color: '#e4283f' }}>
