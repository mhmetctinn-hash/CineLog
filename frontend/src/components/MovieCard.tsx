...
export function MovieCard({ tmdbId, title, posterPath, subtitle, actions }: MovieCardProps) {
  ...
  return (
    <div className="bg-surface rounded-lg overflow-hidden border border-border flex flex-col">
      <Link to={`/movie/${tmdbId}`} className="block aspect-[2/3] bg-base">
        {src ? (
      ...
      <div className="p-3 flex flex-col gap-2 flex-1">
        <Link to={`/movie/${tmdbId}`} className="font-medium text-sm line-clamp-2 hover:text-highlight">
          {title}
        </Link>
